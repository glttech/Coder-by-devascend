...
export default async function RepositoriesPage({ searchParams }: PageProps) {
  ...
  const nextCursor =
    repos.length === PAGE_SIZE ? repos[repos.length - 1].createdAt.toISOString() : null;
  ...
  const addButton = (
    <Link href="/coder/repositories/new" className="btn btn-ghost btn-sm">
      + Add Repository
    </Link>
  );
        ...
        <>
              ...
              <tbody>
                {repos.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="col-hide-mobile" style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                      {r.syncedAt
                        ? r.syncedAt.toLocaleDateString(undefined, {
                            month: 'short',
                            day: 'numeric',
                          })
                        : '—'}
          ...
          {nextCursor && (
            <div style={{ marginTop: 16, textAlign: 'center' }}>
              <Link
                href={`/coder/repositories?cursor=${encodeURIComponent(nextCursor)}&orgId=${orgId}`}
