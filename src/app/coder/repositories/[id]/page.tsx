...
export default async function RepositoryDetailPage({ params }: PageProps) {
  ...
  const prCount = repo.repositoryPRs.length;
  const sessionCount = repo._count.cliSessions;
  ...
  return (
    <div>
      ...
      >
        <PageHeader
          title={repo.fullName}
          subtitle={`${prCount} PR${prCount !== 1 ? 's' : ''} · ${sessionCount} CLI session${sessionCount !== 1 ? 's' : ''}`}
        />
      ...
      >
        ...
        <div>
          ...
          <div>{repo.syncedAt ? repo.syncedAt.toLocaleString() : 'Never'}</div>
        </div>
        {repo.lastSyncError && (
              ...
              {repo.repositoryPRs.map((pr) => (
                <tr key={pr.id}>
                  ...
                  <td className="col-hide-mobile">
                    <code style={{ fontSize: 11 }}>{pr.sourceBranch ?? '—'}</code>
                  </td>
                  <td className="col-hide-mobile">
                  ...
                  <td className="col-hide-mobile" style={{ fontSize: 12 }}>
                    {pr.task ? (
                      <Link href={`/tasks/${pr.task.id}`} style={{ color: 'var(--blue)' }}>
                        {pr.task.title.length > 30
                          ? pr.task.title.slice(0, 30) + '…'
                          : pr.task.title}
                      </Link>
                    ) : (
                  ...
                  <td className="col-hide-mobile" style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                    {pr.githubUpdatedAt
                      ? pr.githubUpdatedAt.toLocaleDateString(undefined, {
                          month: 'short',
                          day: 'numeric',
                        })
                      : '—'}
