...
// GET /api/tasks/[id]/evidence — return the full Change Control Pack for a task.
export async function GET(
  ...
  try {
    ...
    // Fetch GitHub PRs linked to this task via projectId + taskId
    const githubPRs = await prisma.githubPR.findMany({
      where: { projectId: task.projectId, taskId: params.id },
      select: {
        prNumber: true,
        title: true,
        state: true,
